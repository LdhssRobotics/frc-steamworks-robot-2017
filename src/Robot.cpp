...
void Robot::FrontCameraVisionThread() {
...
void Robot::BackCameraVisionThread() {
	// Get the USB camera from CameraServer
	cs::UsbCamera camera = CameraServer::GetInstance()->StartAutomaticCapture();
	// Set the resolution
	camera.SetResolution(160, 120);
...
void Robot::RobotInit() {
	...
	std::thread frontCameraVisionThread(FrontCameraVisionThread);
	frontCameraVisionThread.detach();

	std::thread backCameraVisionThread(BackCameraVisionThread);
	backCameraVisionThread.detach();
